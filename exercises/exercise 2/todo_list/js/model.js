...
class Clowder extends Subject {
    ...
    adopt(list) {
        this._clowder.push(list);
        this.publish("new list added", this);
    }

    remove(list) {
        let value = list;
        this._newList = this._newList.filter(item => item !== value)
        this.publish("Remove Task", this);
    }
    ...
    cleanList() {
        this._clowder = this._clowder.filter(aList => aList.removed);
        this.publish("list(s) removed", this);
    }
